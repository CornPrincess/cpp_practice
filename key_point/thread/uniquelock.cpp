...
public:
    void func1() {
        ...
        cout << "func1 get lock, start to sleep" << endl;
        std::chrono::milliseconds slice(10000);
    ...
    void func2() {
        ...
        // core try to lock
        std::unique_lock<std::mutex> uniqueLock(m_mutex1, std::try_to_lock);
        if (uniqueLock.owns_lock()) {
            cout << "func2 get lock" << endl;
            cout << "func2 " << "thread: " << this_thread::get_id() << endl;
        } else {
            cout << "func2 not get lock, bad lock";
        }
    }
private:
    std::mutex m_mutex1;
